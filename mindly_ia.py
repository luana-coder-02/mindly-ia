...
def clean_markdown(text: str) -> str:
    ...
    text = re.sub(r'^\s*?([•o*\-✓✔✔✅])\s?(.+)', r'- \2', text, flags=re.MULTILINE)
    text = re.sub(r'([a-zA-ZáéíóúÁÉÍÓÚñÑ])\*\s?([a-zA-ZáéíóúÁÉÍÓÚñÑ])', r'\1\2', text)
    text = re.sub(r'\[(.*?)\]\(.*?\)', r'\1', text)
    text = re.sub(r'`{3}.*?`{3}', '', text, flags=re.DOTALL)
    return text.strip()

system_message = """
    Eres Mindly, un chatbot empático y profesional, experto en ayudar a las personas a encontrar información clara sobre psicología.
    Respondes de forma cercana y sin jerga técnica.
...
# Input del usuario
    if prompt := st.chat_input("💭 Comparte lo que está en tu mente..."):
        ...
        if respuesta_final and respuesta_final.strip():
            ...
            st.chat_message("assistant").markdown("Lo siento, tuve un problema técnico y no pude generar una respuesta. Por favor, intenta de nuevo.")
